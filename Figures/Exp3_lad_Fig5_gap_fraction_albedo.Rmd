...
dat = aggregate(albedo ~ height_m + pos + angles + angles2 + num_of_species + species + idSimulation, 
                   data = dat, FUN = "sum")
...
aggdat = aggregate(albedo ~  pos + angles + angles2 + num_of_species, 
...
```{r}
res_tab$both_angles = sapply(c(1:nrow(res_tab)), function(i) {
  ab = sort(c(res_tab$angles[i], res_tab$angles2[i]))
  a = ab[1]
  b = ab[2]
  return(100*a + b)
  })
...
aggdat = aggregate(diff_sum ~ pos + angles + angles2 + both_angles, 
...
aggdat = aggregate(diff_sum ~ both_angles + diff_angles, 
...
ggplot(mt, aes(diff_angles, diff_sum, group = diff_angles, fill = as.factor(med_diffsum))) +       geom_boxplot(width=0.5, alpha = 1, size = 0.65) + 
  ...
  xlab("Gap fraction distance") + ylab("shortwave absorptance change, %") + theme_bw() +
  theme(
