...
levels(dat$species) = c("2", "3", "4", "5", "6", "1")
dat$species = factor(dat$species, levels = c("1", "2", "3", "4", "5", "6"))
...
ann_text_1 = aggregate(abs_mean ~ num_of_species + species + scenetype, data = crown[crown$species %in% c("1", "6") & crown$num_of_species == 1,], FUN = sum)
ann_text_1$abs_mean = round(ann_text_1$abs_mean, 2)
ann_text_6 = aggregate(abs_mean ~ num_of_species + species + scenetype, data = crown[crown$species %in% c("1", "6") & crown$num_of_species == 6,], FUN = sum)
...
ggplot(data = crown[crown$num_of_species %in% c(1,6),], 
  ...
  geom_text(data  = ann_text_1[ann_text_1$species == "1",], mapping = aes(x = -Inf, y = -Inf, label = abs_mean, col = species),
            hjust = -0.4, vjust   = -20.75, show.legend = FALSE, size = 4.5, alpha = 0.6) +
  geom_text(data  = ann_text_6[ann_text_6$species == "1",], mapping = aes(x = -Inf, y = -Inf, label = abs_mean, col = species),
            hjust = -0.4, vjust   = -18.75, show.legend = FALSE, size = 4.5) +
