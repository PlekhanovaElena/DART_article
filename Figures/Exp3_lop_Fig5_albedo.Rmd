...
aggdat = aggregate(albedo ~ pos + transm1 + transm2 + refl1 + refl2 + num_of_species, 
                   data = subdat_crown, FUN = "mean")
res_tab = lapply(unique(aggdat$pos[as.numeric(aggdat$num_of_species) > 1]), function(POS) {

  av_mixture = subset(aggdat, pos == POS, albedo)
  ...
  abs_of_monocultures = lapply(unique(subdat_crown$species[subdat_crown$pos == POS]), function(x) {
    ...
    res = subset(aggdat, pos == position_of_mon, select = albedo)
    return(res)
  })
  ...
  colnames(one_mix) = c("albedo", "height_m", "pos")
  
  one_mix$diff_sum = rep(round(sum(one_mix$albedo), 7), nrow(one_mix))
...
my_palette = brewer.pal(11,"Purples")
...
heatmap.2(t(albmat),margins=c(5,5), scale="none", labCol = sort(unique(vector_b)),
          ...
          dendrogram="none",  trace="none", xlab = "Leaf absorptance distance", key.title = NA, keysize = 1,
          lmat = lmat, lwid = lwid, lhei = lhei,
          key.xlab = "shortwave radiation change, %",
          ylab = "Leaf transmittance distance", density.info="none", Colv = NA, Rowv = NA,
...
heatmap.2(t(albmat),margins=c(7,7), scale="none", labCol = sort(unique(vector_b)),
          ...
          key.xlab = "shortwave absorptance change, %", par = par(cex.lab = 1.8, cex.axis = 1.8),
