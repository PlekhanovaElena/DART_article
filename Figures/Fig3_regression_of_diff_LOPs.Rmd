...
read_profiles = function(trait_loc = trait, WAVE_RANGE_loc = WAVE_RANGE, sun_angle_30_loc = sun_angle_30) {
  ...
  dat = read.csv(
    paste0("./Data_for_figures/profiles_",trait_loc,"6_", angle, ".csv"))
...
ggplot(data = subset(mix_tab, scenetype == "white"), 
  ...
  ylab("FAPAR change")  + theme_bw() + xlab("mixtures of two species") + 
  coord_flip() +
...
mix_tab$abs = 1 - mix_tab$refl - mix_tab$transm
mix_tab$leaf_diff_mon[mix_tab$sp_a == 1] = mix_tab$abs[mix_tab$sp_a == 1] - mix_tab$abs[mix_tab$sp_a == 0]
mix_tab$leaf_diff_mon[mix_tab$sp_a == 0] = - (mix_tab$abs[mix_tab$sp_a == 1] - mix_tab$abs[mix_tab$sp_a == 0])
mix_tab$leaf_diff_mon_transm[mix_tab$sp_a == 1] = mix_tab$transm[mix_tab$sp_a == 1] - mix_tab$transm[mix_tab$sp_a == 0]
mix_tab$leaf_diff_mon_transm[mix_tab$sp_a == 0] = - (mix_tab$transm[mix_tab$sp_a == 1] - mix_tab$transm[mix_tab$sp_a == 0])
mix_tab$leaf_diff_mon_refl[mix_tab$sp_a == 1] = mix_tab$refl[mix_tab$sp_a == 1] - mix_tab$refl[mix_tab$sp_a == 0]
mix_tab$leaf_diff_mon_refl[mix_tab$sp_a == 0] = - (mix_tab$refl[mix_tab$sp_a == 1] - mix_tab$refl[mix_tab$sp_a == 0])
```

```{r fig.height=7, fig.width=5}
ab_mixtab = mix_tab
ab_mixtab$diff_mon[ab_mixtab$sp_a == 0] = - ab_mixtab$diff_mon[ab_mixtab$sp_a == 0]
ggplot(data = subset(ab_mixtab, scenetype == "black"), 
       aes(x = mon_abs, y = abs, col = as.factor(species), group = as.factor(pos)) ) +
  geom_point() +
  ylab("Absorbtion in monoculture")  + theme_bw() + xlab("Absorbtion of a leaf") + 
...
ab_mixtab$diff_mon[ab_mixtab$sp_a == 0] = - ab_mixtab$diff_mon[ab_mixtab$sp_a == 0]
ggplot(data = subset(ab_mixtab, scenetype == "black"), 
       aes(x = abs_mean, y = leaf_diff_mon + leaf_diff_mon_transm, col = as.factor(species), group = as.factor(pos)) ) +
...
ab_mixtab = mix_tab
ab_mixtab$diff_mon[ab_mixtab$sp_a == 0] = - ab_mixtab$diff_mon[ab_mixtab$sp_a == 0]
ggplot(data = subset(ab_mixtab, scenetype == "white"), 
       aes(x = leaf_diff_mon, y = diff_mon, col = as.factor(species), group = as.factor(pos)) ) +
  geom_point() + geom_line() +
  geom_hline(aes(yintercept = 0), col = "gray35") +
  ylab("abs_mean")  + theme_bw() + xlab("Diff in leaf abs of monocultures") + 
  scale_color_brewer(palette = "Dark2", name = NULL) +
...
```{r fig.height=7, fig.width=5}
ggplot(data = subset(mix_tab, scenetype == "white"), 
       aes(x = leaf_diff_mon_refl, y = abs_mean, col = as.factor(species), group = as.factor(pos)) ) +
  geom_point() + geom_line() +
  geom_hline(aes(yintercept = 0), col = "gray35") +
...
ggplot(data = ab_mixtab, 
  ...
  geom_point(size = 1.5) + #geom_line() +
  scale_color_brewer(palette = "Dark2", name = "") +
  ylab("FAPAR change")  + theme_bw() + xlab("FAPAR difference\n in monocultures") + 
