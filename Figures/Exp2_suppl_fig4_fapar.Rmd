...
```{r}
ex = read.csv("Data_for_figures/50nm/profiles_exp1_lai_90.csv")
summary(ex$albedo[ex$band %in% c(1:7) & ex$num_of_species == 1])
```
```{r}
summary(dat$albedo[dat$band %in% c(1:7)])
```
```{r}
summary(dat$albedo[dat$band %in% c(1:7) & dat$num_of_species == 1])
```
...
```
```{r}
summary(dat$albedo[dat$num_of_species == 1])
```


```{r}

dat = aggregate(albedo ~ height_m + num_of_sp1 + num_of_sp2 + species + num_of_species + pos + idSimulation, data = dat, FUN = "sum")

dat = aggregate(albedo ~ num_of_sp1 + num_of_sp2 + species + num_of_species, data = dat, FUN = "mean")
```
...
heatmap.2(t(albmat),margins=c(7,10), scale="none", labCol = unique(vector_b),
          ...
          dendrogram="none",  trace="none", xlab = "LAI richness", 
          ylab = "LAD richness", main = "Albedo", density.info="none", Colv = NA, Rowv = NA,
          col=my_palette)
```

```{r}
mean( subdat$albedo[subdat$diversity_LAD == 1 & subdat$diversity_LOP== 4])
```

```{r}
(mean( subdat$albedo[subdat$diversity_LAD == 4 & subdat$diversity_LOP== 1]) - mean( subdat$albedo[subdat$diversity_LAD == 1 & subdat$diversity_LOP== 1]))/mean( subdat$albedo[subdat$diversity_LAD == 1 & subdat$diversity_LOP== 1])
```
...
ggplot(data = subdat, 
  ...
  ylab("albedo") + xlab("LAI richness") + theme_minimal() +
